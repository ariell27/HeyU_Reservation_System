 ...
 * 获取可用时间槽（从后端）
 * @param {string} date - 日期字符串 (YYYY-MM-DD)
 * @param {Object} service - 服务对象（包含 duration 字段）
 * @returns {Promise<Array<string>>} 可用时间槽数组
 */
export async function getAvailableTimeSlots(date, service) {
  try {
    const response = await fetch(`${API_URL}/api/time-slots/available`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.message || '获取可用时间槽失败');
    }
...
/**
 * 获取默认时间槽（从后端，不考虑预订和 block）
 * @param {string} date - 日期字符串 (YYYY-MM-DD)
 * @returns {Promise<Array<string>>} 默认时间槽数组
 */
export async function getDefaultTimeSlots(date) {
  try {
    const params = new URLSearchParams({ date });
    const response = await fetch(`${API_URL}/api/time-slots/default?${params.toString()}`);
    
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.message || '获取默认时间槽失败');
    }
